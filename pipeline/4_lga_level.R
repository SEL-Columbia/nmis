...
education_mopup_lga_indicators = function(education_data) {
    ...
    primary_and_junior_sec_indicators = function(data, level) {
        stopifnot(level %in% c("primary", "junior"))
        data <- data %.% 
            group_by(lga) %.% 
        ...
        # rename all but the grouping (ie, lga) column
        names(data)[2:ncol(data)] <- paste(names(data)[2:ncol(data)], level_suffix, sep="_")
        data
    }
...
health_mopup_lga_indicators = function(health_data) {
    health_data = health_data %.% mutate(
        ## Some definitions to help us make indicators later on
        is_public = management %in% c('federal_gov', 'local_gov', 'state_gov'),
    ...
    #    alternative_functional_power = # either have functional generator, or funcitonal solar
    ...
    allExceptHealthPost_data = health_data %.% filter(is_allExceptHealthPost) %.% group_by(lga) %.%
        dplyr::summarise(
            ## Services that are provided at all facilties except for Health Posts
...
#             proportion_power_alternative_functional = mean(alternative_functional_power, na.rm=T))
     allFacilities_data %.% left_join(allExceptHealthPost_data, by='lga') %.% left_join(hospital_data, by='lga')
}
