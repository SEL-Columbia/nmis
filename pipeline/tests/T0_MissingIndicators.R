...
test_that('Required Facility Level Indicators are not missing.', {
    ...
    (e_miss <- missing_indicators(readRDS(output_file("Education_mopup_NMIS_Facility.rds")), 
        required_indicators$facility, 'education'))
    (h_miss <- missing_indicators(readRDS(output_file("Health_mopup_NMIS_Facility.rds")), 
...
test_that('Required LGA Indicators are not missing.', {
    output_file <- function(file) { sprintf("%s/%s", CONFIG$OUTPUT_DIR, file) }
    (e_miss <- missing_indicators(readRDS(output_file("Education_mopup_LGA_Aggregations.rds")), 
        required_indicators$lga, 'education'))
    (h_miss <- missing_indicators(readRDS(output_file("Health_mopup_LGA_Aggregations.rds")), 
