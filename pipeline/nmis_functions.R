...
get_necessary_indicators <- function() {
    ...
    facility_level_extras = c("formhub_photo_id", "facility_name", "gps", "survey_id", "unique_lga", "latitude", "longitude")
...
xxx_replace <- function(vec){
    return(ifelse(grepl("^(x|\\s){3,}$", vec, ignore.case = T, perl = T), NA, vec))
}
